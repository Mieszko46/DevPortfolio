...
void Animation::SetupBones(const aiAnimation* animation, Model& model)
{
	...
	auto& boneInfoMap = model.GetBoneOffsetMatMap();
...
void Animation::ReadHierarchyData(AssimpNodeData& path, const aiNode* src)
{
	assert(src);

	path.name = src->mName.data;
	path.transformation = AssimpToGLMConvert::ConvertMatrixToGLMFormat(src->mTransformation);
	path.childrenCount = src->mNumChildren;

	for (int i = 0; i < src->mNumChildren; i++) {
		AssimpNodeData newData;
		ReadHierarchyData(newData, src->mChildren[i]);
		path.children.push_back(newData);
	}
}
